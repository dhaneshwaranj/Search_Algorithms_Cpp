...
string dfs_search(std::vector<std::vector<char>>& maze, pi start, pi goal, unordered_map<char,pi>& moves);
...
string dfs_search(vector<vector<char>>& maze, pi start, pi goal, unordered_map<char,pi>& moves){
...
// actions to be taken to get to the goal
	...
	while(!frontier.empty()){
		...
		// For DFS pop the frontier in a LIFO manner
		FrontierNode frontier_node = frontier.top();
		frontier.pop();
